describe('Checking out a user', function() {
  it('user can checkout ', function(){
    ...
    cy.get('input[name=address1]').type('1 Davidson Road')
    .should('have.value', '1 Davidson Road');
    cy.get('input[name=address2]').type('South Croydon')
    .should('have.value', 'South Croydon');
    cy.get('input[name=town]').type('London')
    .should('have.value', 'London');
    cy.get('input[name=Postcode]').type('CR8 6EN')
